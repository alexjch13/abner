$(document).ready(function(){
	var url="http://192.168.1.4:80/LOGIN/www/auth.php";
    	...
    	if($.trim(fullname).length>0 & $.trim(email).length>0 & $.trim(password).length>0)
		{
			$.ajax({
				...
				success: function(data){
					if(data=="success")
					{
						alert("Gracias por registrarce! ya puede chatear");
					}
					else if(data="exist")
					{
    	...
    	if($.trim(email).length>0)
		{
			$.ajax({
				...
				success: function(data){
					if(data=="invalid")
					{
						alert("Tu no te has registrado aun");
					}
					else if(data="success")
					{
						alert("Hemos enviado la contrasenna a tu correo, por favor revisalo");
					}
				}
			});
		}return false;
